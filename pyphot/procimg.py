...
def detproc(scifiles, camera, det, n_process=4, science_path=None, masterbiasimg=None, masterdarkimg=None, masterpixflatimg=None,
            ...
            maskbrightstar=True, brightstar_nsigma=3, erosion=11, dilation=50, maskbrightstar_method='sextractor',
            conv='sex', sextractor_task='sex', verbose=True, overwrite=True):
    ...
    if n_process == 1:
        for ii, scifile in enumerate(scifiles):
            sci_fits_file, flag_fits_file = _detproc_one(scifile, camera, det,
                        ...
                        erosion=erosion, dilation=dilation, maskbrightstar_method=maskbrightstar_method, conv=conv,
        ...
        for w in range(n_process):
            p = Process(target=_detproc_worker, args=(work_queue, ), kwargs={
                ...
                'maskbrightstar':maskbrightstar, 'brightstar_nsigma':brightstar_nsigma,
                'erosion': erosion, 'dilation': dilation, 'maskbrightstar_method':maskbrightstar_method,
...
def _detproc_one(scifile, camera, det, science_path=None, masterbiasimg=None, masterdarkimg=None, masterpixflatimg=None,
                 ...
                 erosion=11, dilation=50, sextractor_task='sex', verbose=True, overwrite=True):
    ...
    if os.path.exists(sci_fits_file) and not overwrite:
        ...
        if maskbrightstar:
            ...
            starmask = mask_bright_star(sci_image, mask=bpm_for_wht, brightstar_nsigma=brightstar_nsigma,
                                        ...
                                        erosion=erosion, dilation=dilation, verbose=verbose)
...
def _detproc_worker(work_queue, science_path=None, masterbiasimg=None, masterdarkimg=None, masterpixflatimg=None,
                    masterillumflatimg=None, bpm_proc=None, mask_vig=False, minimum_vig=0.5, apply_gain=False, grow=1.5,
                    maskbrightstar=True, brightstar_nsigma=3, erosion=11, dilation=50,
                    maskbrightstar_method='sextractor', conv='sex', sextractor_task='sex', verbose=True, overwrite=True):
    ...
    while not work_queue.empty():
        ...
        sci_fits_file, flag_fits_file = _detproc_one(scifile, camera, det,
                        ...
                        apply_gain=apply_gain, grow=grow, erosion=erosion, dilation=dilation,
...
def sciproc(scifiles, flagfiles, n_process=4, airmass=None, coeff_airmass=0., mastersuperskyimg=None, use_medsky=False,
            ...
            maskbrightstar=True, brightstar_nsigma=3, erosion=11, dilation=50,
            maskbrightstar_method='sextractor', conv='sex', sextractor_task='sex',
    ...
    if n_process == 1:
        for ii, scifile in enumerate(scifiles):
            sci_fits_file, wht_fits_file, flag_fits_file = _sciproc_one(scifile, flagfiles[ii],
                ...
                back_maxiters=back_maxiters, grow=grow, maskbrightstar=maskbrightstar, erosion=erosion,
                dilation=dilation, brightstar_nsigma=brightstar_nsigma, maskbrightstar_method=maskbrightstar_method,
                conv=conv, sextractor_task=sextractor_task, mask_cr=mask_cr, contrast=contrast,
                lamaxiter=lamaxiter, sigclip=sigclip, cr_threshold=cr_threshold,
        ...
        if maskbrightstar:
            if os.path.exists(star_fits_file):
                ...
                starmask = mask_bright_star(data, mask=bpm_for_star, brightstar_nsigma=brightstar_nsigma, back_nsigma=sigclip,
                                            ...
                                            conv=conv, erosion=erosion, dilation=dilation, verbose=verbose)
                starmask = grow_masked(starmask, grow, verbose=verbose)
                io.save_fits(star_fits_file, starmask.astype('int32'), header, 'FLAG', overwrite=True)
...
def _sciproc_worker(work_queue, coeff_airmass=0., mastersuperskyimg=None, use_medsky=False,
    ...
    while not work_queue.empty():
        ...
        sci_fits_file, wht_fits_file, flag_fits_file = _sciproc_one(scifile, flagfile, airmass,
            ...
            back_maxiters=back_maxiters, grow=grow, maskbrightstar=maskbrightstar, brightstar_nsigma=brightstar_nsigma,
            erosion=erosion, dilation=dilation, maskbrightstar_method=maskbrightstar_method, conv=conv,
            sextractor_task=sextractor_task, mask_cr=mask_cr, contrast=contrast, lamaxiter=lamaxiter,
            sigclip=sigclip, cr_threshold=cr_threshold, neighbor_threshold=neighbor_threshold, mask_sat=mask_sat,
            sat_sig=sat_sig, sat_buf=sat_buf, sat_order=sat_order, low_thresh=low_thresh, h_thresh=h_thresh,
            small_edge=small_edge, line_len=line_len, line_gap=line_gap, percentile=percentile,
            mask_negative_star=mask_negative_star, replace=replace, verbose=verbose, overwrite=overwrite)
