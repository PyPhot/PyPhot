...
def _detproc_one(scifile, camera, det, science_path=None, masterbiasimg=None, masterdarkimg=None, masterpixflatimg=None,
    ...
    if os.path.exists(sci_fits_file) and not overwrite:
        ...
        # save flag image
        flag_image = bpm*np.int32(2**0) + bpm_proc*np.int32(2**1) + bpm_sat*np.int32(2**2) + \
                     bpm_zero * np.int32(2**3) + bpm_nan*np.int32(2**4) + bpm_vig*np.int32(2**5)
...
def _sciproc_one(scifile, flagfile, airmass, coeff_airmass=0., mastersuperskyimg=None, use_medsky=False,
    ...
    if os.path.exists(sci_fits_file) and not overwrite:
        ...
        flag_image_new = flag_image + bpm_cr.astype('int32')*np.int32(2**6) + bpm_sat.astype('int32')*np.int32(2**7)
        flag_image_new += bpm_negative.astype('int32')*np.int32(2**8)
