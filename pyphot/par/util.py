...
#-----------------------------------------------------------------------
# Functions for parsing the input pyphot file
# TODO: Should these go into a different module? PypitSetup?
#-----------------------------------------------------------------------
def _read_pyphot_file_lines(ifile):
    """
...
def _find_pyphot_block(lines, group):
    """
...
def parse_pyphot_file(ifile, file_check=True, runtime=False):
    """
    ...
    """
    # Read in the pyphot reduction file
    msgs.info('Loading the reduction file')
    lines = _read_pyphot_file_lines(ifile)
    ...
    # Parse data block
    s, e = _find_pyphot_block(lines, 'data')
    if s >= 0 and e < 0:
    ...
    s, e = _find_pyphot_block(lines, 'setup')
    if s >= 0 and e < 0:
    ...
    # TODO: This should be moved to the PyPhot class
    # Running PyPhot?
    if runtime:
        for key in ['filename', 'frametype']:
            if key not in usrtbl.keys():
                msgs.error("Add {:s} to your PyPhot file before using run_pyphot".format(key))
        # Setup
        if len(setups) != 1:
            msgs.error("Add setup info to your PyPhot file in the setup block!")

    msgs.info('Input file loaded successfully')
    return list(lines[is_config]), data_files, frametype, usrtbl, setups


def pyphot_config_lines(ifile):
    """
...
def make_pyphot_file(pyphot_file, spectrograph, data_files, cfg_lines=None, setup_mode=False,
    ...
    # Here we go
    with open(pyphot_file, 'w') as f:
        f.write('# Auto-generated PyPhot file\n')
    ...
    msgs.info('PyPhot file written to: {0}'.format(pyphot_file))
